...
- **영어·전문 용어를 한 선택지에 몰지 않는다.** 한 선택지에만 영어 단어, 약어, 코드명, 괄호 보충(예: `(speech synthesis)`, `(Owner)`)이 들어가면 정답이 즉시 드러난다. 네 선택지에 비슷한 빈도로 분포시키거나 모두 한국어로 통일한다.

#### 선택지 검수 체크리스트

문항을 작성·수정한 뒤 항상 다음 6가지를 통과시킨다. 한 항목이라도 어긋나면 다듬는다.

1. **길이**: 가장 긴 보기와 가장 짧은 보기의 글자 수 비율이 1.5배 이내인가? 정답이 1.5배 넘게 길면 오답에도 디테일을 더해 균형을 맞춘다.
2. **어미·구조**: 네 보기의 어미가 같은 형태(`~한다`/`~된다`/명사형 중 하나)로 통일되어 있는가?
3. **절대어 분산**: "모든·전혀·절대·자동으로·영구적으로·수동으로·반드시·오직" 같은 표현이 정답·오답 진영 한쪽에만 몰려 있지 않은가?
4. **영어·약어 분배**: 영어 단어, 약어, 괄호 보충(`(...)`), 영문 코드명이 한 선택지에만 등장하지 않는가? 분산이 어려우면 모두 한국어로 통일한다.
5. **본문 인용 분배**: 강의 본문의 정확한 표현·고유명사·코드명이 정답 보기에만 그대로 등장하지 않는가? 정답에서는 풀어 쓰고, 정확한 원문은 `explanation`으로 옮긴다.
6. **그럴듯한 오답**: 네 보기 중 "개념을 몰라도 누가 봐도 말이 안 되는" 보기가 있는가? 있다면 실제 비슷한 제품에서 가능했을 법한 동작이거나 학습자가 오해할 만한 설명으로 다듬는다.
