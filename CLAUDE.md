...
1. **미디어 자산 수집.** `.venv/bin/python fetch_lesson_videos.py [lesson_index]`로 해당 레슨 페이지의 iframe src와 본문 영역 `<img>` URL을 한 번에 뽑아낸다. 출력에 YouTube embed가 있으면 ID(`/embed/` 뒤 11자)를 메모하고, 이미지 URL은 본문에 등장하는 순서대로 정리해 둔다. 영상이 0개여도 — 이미지가 0장이어도 — 스크립트로 직접 확인한 결과를 근거로 삼는다.
