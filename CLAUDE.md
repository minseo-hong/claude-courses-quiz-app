...
- **Slug 생성 규칙** (`src/lib/quizzes.ts`):
  ...
  - **한국어·특수문자 타이틀은 데이터 파일에 `slug` 필드를 명시**한다. `QuizModule.slug`가 지정되면 slugify 결과보다 우선한다 (예: `title: '1회 모의고사'` + `slug: 'mock-exam-1'`). URL에 한국어가 들어가면 인코딩·공유·디버깅이 번거롭다.
...
export default {
  ...
  title: '1회 모의고사',    // 사이드바·헤더에 표시되는 제목
  slug: 'mock-exam-1',     // URL slug. 한국어·특수문자 타이틀이면 반드시 명시
  questions: [
    {
      id: 1,
...
export default [
  ...
  { title: "Expanding Claude's reach", lessons: ['08', '09', '10'] },
