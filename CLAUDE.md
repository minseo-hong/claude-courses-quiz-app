...
├── skilljar.py                             # Skilljar Playwright CLI (text·markdown·markdown-all·sidebar·media 서브커맨드)
...
Skilljar 관련 작업은 단일 CLI `skilljar.py`로 통합되어 있다. 모든 서브커맨드는 `cookies.json`을 사용하며, 시작 URL은 `--start-url`로 덮어쓸 수 있다 (기본: Claude 101 첫 페이지).

```bash
.venv/bin/python skilljar.py text <강의_URL>          # 본문 plain text
.venv/bin/python skilljar.py markdown <강의_URL>      # 본문 Markdown
.venv/bin/python skilljar.py markdown-all             # 전체 레슨을 markdown/{NN}-{slug}.md로 저장
.venv/bin/python skilljar.py sidebar                  # 사이드바 H3-그룹 구조 JSON
.venv/bin/python skilljar.py media <lesson_index>     # N번째 레슨의 iframe·video·image URL
```

- `markdown-all`은 `markdown/` 폴더(저작권 보호를 위해 git 제외)에 원문 마크다운을 일괄 저장한다. 쿠키 갱신 후 재실행하면 최신 상태가 된다.
- `sidebar`는 강의 구조가 바뀌었을 때 `sections.js` 업데이트의 근거가 된다.
- `media`는 한국어 번역에 임베드할 YouTube ID·이미지 URL을 한 번에 뽑아낸다.
...
새 레슨을 한국어로 추가할 때는 **원문 강의의 모든 YouTube 영상과 이미지를 빠짐없이 포함**한다 — 텍스트만 번역해 두는 것은 허용하지 않는다. 본문 흐름에서 미디어가 빠지면 학습 경험이 원문과 달라지므로, 다음 두 단계를 *항상* 거친다.

1. **미디어 자산 수집.** `.venv/bin/python skilljar.py media <lesson_index>`로 해당 레슨 페이지의 iframe src와 본문 영역 `<img>` URL을 한 번에 뽑아낸다. 출력에 YouTube embed가 있으면 ID(`/embed/` 뒤 11자)를 메모하고, 이미지 URL은 본문에 등장하는 순서대로 정리해 둔다. 영상이 0개여도 — 이미지가 0장이어도 — 스크립트로 직접 확인한 결과를 근거로 삼는다.
2. **본문에 원위치 그대로 배치.** 텍스트 번역과 함께 영상은 `<!-- youtube: VIDEO_ID -->` 마커, 이미지는 `![alt](url)` 마크다운으로 *원문에서 등장한 섹션·단락에 그대로* 끼워 넣는다. 위치를 헤더 영역으로 옮기거나 별도 갤러리로 모으지 않는다 — 사용자가 원문 흐름과 동일한 시각 자료를 같은 자리에서 만나야 한다.

레슨에 자산이 정말 하나도 없다면 그 자체로 OK다. 다만 *확인 없이 생략*하지 않는다 — 누락이면 PR 리뷰 단계에서 잡혀야 한다.
...
- **YouTube 영상 임베드 마커**는 영상이 실제로 등장하는 본문 위치에 `<!-- youtube: VIDEO_ID -->` 한 줄로 둔다. `MarkdownContent`가 마커 자리에서 본문을 분할해 그 자리에 16:9 iframe(`https://www.youtube.com/embed/{VIDEO_ID}`)을 렌더하고, 마커 앞·뒤 마크다운은 별도 블록으로 그대로 렌더된다. 영상 URL을 모를 때는 `skilljar.py media <lesson_index>`로 해당 Skilljar 레슨 페이지의 iframe src를 뽑아 ID만 떼어 쓴다.
- **본문 이미지**는 표준 마크다운 `![alt](url)` 문법으로 영상과 동일하게 *원본 레슨에서의 위치*에 그대로 둔다. 외곽 카드·테두리·둥근 모서리 같은 장식은 입히지 않는다 — `markdownComponents.tsx`의 `img` 매핑이 `block max-w-full h-auto`만 적용해 일반 웹페이지처럼 자연스럽게 본문 흐름에 녹는다. 이미지 URL은 Skilljar 원문에서 가져온 외부 CDN 주소를 그대로 사용해도 무방하며, `skilljar.py media`가 본문 영역 한정으로 `<img>` src를 함께 출력해 준다.
