...
└── src/
    ...
    ├── lib/
    │   ├── urls.ts                         # URL 헬퍼 (courseSlug, lessonContentPath, lessonQuizPath) — 양쪽에서 import 가능
...
- **URL 헬퍼는 분리**: `lib/urls.ts`에 `courseSlug`·`lessonContentPath`·`lessonQuizPath`만 두어 클라이언트/서버 양쪽에서 자유롭게 import할 수 있도록 한다. 헬퍼가 `lib/quizzes.ts`를 import하면 퀴즈 데이터가 클라이언트 번들에 끌려가므로 금지한다.
...
| 리소스 | 경로 | 파일 |
|---|---|---|
| 루트 | `/` | `src/app/page.tsx` (→ `redirect(lessonQuizPath(firstSlug))`) |
...
- **URL 조립 헬퍼** (`src/lib/urls.ts` — 양쪽에서 import 가능):
  ...
  - `lessonContentPath(slug)`: 강의 내용 URL — plain slug 경로
  - `lessonQuizPath(slug)`: 복습 퀴즈 URL — `lessonContentPath(slug) + '/quiz'`
  - 루트 `/`는 L01에 콘텐츠 번역이 없어 `lessonQuizPath(firstSlug)`로 리다이렉트
- **링크·네비게이션**: `next/link`의 `<Link>`는 뷰포트 진입 시 자동 prefetch된다. `usePathname`/`useParams`는 `next/navigation`에서 가져온다.
- **주요 export**
  - `src/lib/quizzes.ts`: `grouped`(섹션별 퀴즈 — 전체 문항 포함), `quizBySlug`, `firstSlug`, `sidebarSections`(콘텐츠가 있는 레슨은 `content`/`quiz` 두 항목으로 펼쳐짐). 내부에서 `content.ts`의 `hasContent`를 사용하므로 서버 컨텍스트에서만 import 가능(클라이언트는 type-only import만 허용).
